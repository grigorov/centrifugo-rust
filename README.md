...
Сравнение этой реализации с настоящим Centrifugo v2.8.6 (Go) под идентичной нагрузкой. Бенчмарк — `conformance/tests/perf.rs` (помечен `#[ignore]`):

```bash
# release-бинарь — для честного сравнения с оптимизированным Go
...
Два показателя (методика одинаковая для обоих движков, поэтому значимо *соотношение*, а не абсолют):

- **Пропускная способность fan-out** — 100 подписчиков на одном канале, 500 публикаций через HTTP API → 50 000 доставок; скорость = доставки / wall-clock.
- **Задержка broadcast** — один подписчик, медиана/p95 от вызова publish до доставки.

Измерено на **MacBook (Apple M4 Pro, 12 ядер, 24 ГБ RAM)**, медиана 3 прогонов, memory-движок (одноузловой fan-out, apples-to-apples):
...
- **B1** — live-публикации на recoverable-каналах отдают seq/gen (offset обнулён), как Go при `UseSeqGen` (дефолт v2.8.6);
- **B2** — cross-node Disconnect переносит `reconnect` + `whitelist` (важно для `user_personal_single_connection`);
- **B3** — refresh-прокси работает server-side: проактивное обновление по таймеру + client REFRESH отклоняется (3003), как в Go;
- **B4** — AUTH/db для мастера, найденного через Sentinel, + конфиг `redis_password`/`redis_db`;
- **B5** — серверные unsubscribe/disconnect применяются локально до публикации + пропуск собственного loopback по uid;
- **B6/B8/B9/B10/B11** — паритет строк/кодов API (apikey ровно из 2 полей; метод `rpc` → 107/104; gRPC-сообщение `unauthenticated`; Content-Type эхом; комментарий decode_control);
