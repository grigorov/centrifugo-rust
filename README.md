...
- **Принудительное истечение по таймеру.** Истечение соединения/подписки управляется client-refresh; серверный таймер, отключающий истёкшие подписки (`DisconnectSubExpired`), пока не подключён.
...
Все этапы M0–M12 плюс фазы полного паритета (server-side каналы, SUB_REFRESH, `#`-каналы, TTL presence + таймер обновления, гранулярные прокси, Protobuf HTTP API, разрешение на публикацию, Redis Sentinel, admin web UI) завершены. **170 тестов проходит** (юнит + конформанс), 0 падений. Каждое проводное поведение сверено с настоящим Centrifugo v2.8.6 (golden-диффы) и подтверждено живым SDK centrifuge-go. После сквозного аудита совместимости устранены 17 расхождений с эталоном на Go.
