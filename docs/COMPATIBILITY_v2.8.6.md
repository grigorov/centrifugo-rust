...
- **Origin checks & limits (honored):** `allowed_origins` is enforced on the WebSocket upgrade **and** the SockJS transport (glob patterns, case-insensitive; mismatch → HTTP 403); `channel_max_length` (255) and `client_channel_limit` (128) are enforced at SUBSCRIBE (`106`); `client_user_connection_limit` caps connections per authenticated user (`3013`); `history_disable_for_client` makes client HISTORY return `108`. `channel_max_length`, `client_channel_limit` and `allowed_origins` are also overridable via `CENTRIFUGO_*` env (Go binds them with `viper.BindEnv`; `CENTRIFUGO_ALLOWED_ORIGINS` is whitespace-split, matching viper's `GetStringSlice`). `client_user_connection_limit` is config-file/default only — Go does **not** bind it to env, so neither do we. For the five bool flags (`--client_insecure`/`--api_insecure`/`--grpc_api`/`--admin`/`--admin_insecure`), full viper `flag > env > file` precedence holds: an explicitly-set CLI flag wins, else a present `CENTRIFUGO_*` env var overrides the file (either value, incl. `false`), else the file value.
